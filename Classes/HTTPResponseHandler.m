...
- (NSString*)stringByReplacingPercentEscapes:(NSString*)str
{
...
- (NSData*)jsonDataFormRequestParams
{
    NSString* param = nil;
    if ([requestMethod isEqualToString:@"GET"]) {
        param = url.absoluteString;
    }
    else if([requestMethod isEqualToString:@"POST"]) {
        CFDataRef body = CFHTTPMessageCopyBody(request);
        param =  [[NSString alloc] initWithData:(__bridge NSData*)body encoding:NSUTF8StringEncoding];
        CFRelease(body);
    }
    ...
    for (NSTextCheckingResult* resutl in array) {
        NSString* key = [searchText substringWithRange:[resutl rangeAtIndex:1]];
        NSString* value = [searchText substringWithRange:[resutl rangeAtIndex:2]];
        [dictionary setObject:[self stringByReplacingPercentEscapes:value] forKey:key];
    }
    
    [dictionary setObject:self.realtime forKey:@"date"];
